...
function VouchersContent() {
  ...
  const upcomingVouchers = vouchers.filter((v) => new Date(v.startDate) > new Date())
  const ongoingVouchers = vouchers.filter((v) => {
    const now = new Date()
    return new Date(v.startDate) <= now && new Date(v.endDate) >= now
  })
  const endedVouchers = vouchers.filter((v) => new Date(v.endDate) < new Date())
    ...
    return (
      <div className='flex items-center justify-between p-4 border-t border-[#004643]/10 bg-white rounded-b-md'>
        <span className='text-sm text-muted-foreground'>
          Hiển thị {Math.min(currentPage * itemsPerPage, totalItems)} trên {totalItems} kết quả
        </span>
      ...
      <Table>
        <TableHeader className='bg-[#f0f7f6]'>
          <TableRow>
            ...
            <TableHead className='text-[#004643] font-semibold'>Sản phẩm áp dụng</TableHead>
          ...
          {data.length > 0 ? (
            data.map((voucher) => (
              <TableRow key={voucher.id} className='hover:bg-[#f8fbfa]'>
                ...
                <TableCell>
                  <Badge variant='outline' className='border-[#004643]/20 text-[#004643] bg-[#e6f0ef]'>
                    Mã shop
                  </Badge>
                </TableCell>
            ...
            <TableRow>
              <TableCell colSpan={showUsed ? 11 : 10} className='h-24 text-center'>
      ...
      {isCreating ? (
        ...
        <>
          ...
          <div className='flex items-center justify-between'>
            <div>
              <h1 className='text-3xl font-bold text-[#004643]'>Quản lý Vouchers</h1>
          ...
          <div className='max-w-[1200px] grid grid-cols-1 md:grid-cols-2 gap-6'>
            <Card className='border-l-4 border-l-[#004643] bg-white shadow-sm hover:shadow-md transition-shadow'>
              <CardHeader className='pb-2'>
                <CardTitle className='text-lg font-semibold text-[#004643]'>Vouchers Giảm Giá</CardTitle>
              </CardHeader>
              <CardContent className='flex justify-between items-center'>
                <div className='text-sm text-[#004643]/70 max-w-[240px] font-semibold'>
                  Tạo mã giảm giá áp dụng cho tất cả các sản phẩm có trong cửa hàng của bạn hoặc chỉ cho một/một vài sản
                  phẩm nhất định.
                </div>
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    <TabsContent value='ended'>
                      <VoucherTable data={getPaginatedData(endedVouchers)} totalCount={endedVouchers.length} showUsed />
                    </TabsContent>
                  </>
                )}
              </div>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent className='bg-white'>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc chắn muốn xóa voucher <span className='font-semibold'>&quot;{voucherToDelete?.name}&quot;</span> (Mã: {voucherToDelete?.code})? 
