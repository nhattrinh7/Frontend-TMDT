...
import { CalendarIcon, Package, Clock, Search, ChevronLeft, ChevronRight, X } from 'lucide-react'
...
import { getVoucherDetailAPI, updateShopVoucherAPI } from '~/apiRequests/voucher.apiRequest'
...
type VoucherUpdateFormValues = z.infer<typeof voucherUpdateSchema>

interface UpdateShopVoucherFormProps {
  voucherId: string
  onClose: () => void
  onSuccess?: () => void
}
...
export function UpdateShopVoucherForm({ voucherId, onClose, onSuccess }: UpdateShopVoucherFormProps) {
  const [selectedProducts, setSelectedProducts] = useState<string[]>([])
  const [selectedProductDetails, setSelectedProductDetails] = useState<ProductWithVariants[]>([])
  ...
  const form = useForm<VoucherUpdateFormValues>({
    resolver: zodResolver(voucherUpdateSchema),
    defaultValues: {
      code: '',
      name: '',
      description: '',
  ...
  useEffect(() => {
    const fetchVoucherDetail = async () => {
      try {
        ...
        const data = await getVoucherDetailAPI(voucherId)
        ...
        form.reset({
          ...
          scope: data.scope === 'CATEGORY' ? 'ALL' : data.scope as 'ALL' | 'PRODUCT',
          selectedProducts: data.productIds || [],
        })
  ...
  const handleConfirmSelection = () => {
    ...
    const selectedDetails = products.filter(p => tempSelectedProducts.includes(p.id))
    setSelectedProductDetails(prev => {
      const existingIds = new Set(prev.map(p => p.id))
      const newProducts = selectedDetails.filter(p => !existingIds.has(p.id))
      return [...prev.filter(p => tempSelectedProducts.includes(p.id)), ...newProducts]
    })
  ...
  const onSubmit = async (data: VoucherUpdateFormValues) => {
    try {
      const dataToUpdate = {
        code: data.code,
        name: data.name,
        ...
        perUserLimit: data.perUserLimit,
        scope: data.scope,
        selectedProducts: data.scope === 'PRODUCT' ? data.selectedProducts : undefined,
      }
      
      await updateShopVoucherAPI(voucherId, dataToUpdate)
  ...
  return (
    <Card className='max-w-4xl mx-auto border-[#004643]/20 shadow-lg bg-white'>
      <CardHeader className='bg-[#f0f7f6] border-b border-[#004643]/10'>
        <CardTitle className='text-2xl font-bold text-[#004643]'>Cập Nhật Voucher</CardTitle>
        <CardDescription className='text-[#004643]/70 font-semibold'>
          Chỉnh sửa thông tin voucher giảm giá cho cửa hàng
        </CardDescription>
      </CardHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent className='bg-white'>
                      <SelectItem value='ALL' className='text-gray-900'>Toàn bộ sản phẩm</SelectItem>
                      <SelectItem value='PRODUCT' className='text-gray-900'>Sản phẩm cụ thể</SelectItem>
                    </SelectContent>
                  </Select>
                  <FormDescription className='text-xs'>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogContent className='max-w-2xl max-h-[80vh] flex flex-col bg-white'>
                <DialogHeader>
                  <DialogTitle className='text-[#004643]'>Chọn sản phẩm áp dụng voucher</DialogTitle>
                  <DialogDescription>
