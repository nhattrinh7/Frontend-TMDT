...
const szoneVoucherUpdateSchema = z
  .object({
    code: z
      .string()
...
export function UpdateSzoneVoucherForm({
  voucherId,
  onClose,
  onSuccess,
}: UpdateSzoneVoucherFormProps) {
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [categories, setCategories] = useState<Category[]>([]);
  const [categoriesLoading, setCategoriesLoading] = useState(false);
  const [voucherLoading, setVoucherLoading] = useState(true);
  const [voucherData, setVoucherData] = useState<VoucherDetailResponse | null>(
    null,
  );
  ...
  useEffect(() => {
    if (
      (watchScope === "CATEGORY" || voucherData?.scope === "CATEGORY") &&
      categories.length === 0
    ) {
      fetchCategories();
    }
