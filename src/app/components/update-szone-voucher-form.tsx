...
import { updateSzoneVoucherAPI, getVoucherDetailAPI, VoucherDetailResponse } from '~/apiRequests/voucher.apiRequest'
import { getRootCategoriesAPI } from '~/apiRequests/category.apiRequest'
import { toast } from 'sonner'

// Schema validation với Zod
const szoneVoucherUpdateSchema = z.object({
  code: z.string()
...
type SzoneVoucherUpdateFormValues = z.infer<typeof szoneVoucherUpdateSchema>
...
interface UpdateSzoneVoucherFormProps {
  voucherId: string
  onClose: () => void
  onSuccess?: () => void
}
...
export function UpdateSzoneVoucherForm({ voucherId, onClose, onSuccess }: UpdateSzoneVoucherFormProps) {
  const [selectedCategories, setSelectedCategories] = useState<string[]>([])
  const [categories, setCategories] = useState<Category[]>([])
  const [categoriesLoading, setCategoriesLoading] = useState(false)
  const [voucherLoading, setVoucherLoading] = useState(true)
  const [voucherData, setVoucherData] = useState<VoucherDetailResponse | null>(null)

  const form = useForm<SzoneVoucherUpdateFormValues>({
    resolver: zodResolver(szoneVoucherUpdateSchema),
    defaultValues: {
      code: '',
      name: '',
      description: '',
    ...
    const fetchVoucherDetail = async () => {
      try {
        ...
        form.reset({
          ...
          scope: data.scope === 'PRODUCT' ? 'ALL' : data.scope,
  ...
  // Fetch categories when scope changes to CATEGORY or initial load
  ...
  useEffect(() => {
    if ((watchScope === 'CATEGORY' || (voucherData?.scope === 'CATEGORY')) && categories.length === 0) {
      fetchCategories()
    }
  }, [watchScope, voucherData?.scope, categories.length, fetchCategories])
  ...
  const onSubmit = async (data: SzoneVoucherUpdateFormValues) => {
    try {
      const dataToUpdate = {
        code: data.code,
        name: data.name,
      ...
      await updateSzoneVoucherAPI(voucherId, dataToUpdate)
      toast.success('Cập nhật Szone voucher thành công!')
      onSuccess?.()
      onClose()
    } catch {
      toast.error('Không thể cập nhật Szone voucher')
    }
  }
  ...
  return (
    <Card className='max-w-4xl mx-auto border-[#004643]/20 shadow-lg bg-white'>
      <CardHeader className='bg-[#f0f7f6] border-b border-[#004643]/10'>
        <CardTitle className='text-2xl font-bold text-[#004643]'>Cập Nhật Szone Voucher</CardTitle>
        <CardDescription className='text-[#004643]/70 font-semibold'>
          Chỉnh sửa thông tin voucher giảm giá toàn sàn
        </CardDescription>
      </CardHeader>
