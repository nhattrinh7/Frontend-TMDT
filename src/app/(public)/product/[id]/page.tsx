...
export default function ProductDetailPage() {
  ...
  const [selectedVariant, setSelectedVariant] = useState<ProductVariantToSold | null>(null)
  ...
  // Reviews state
  const [reviews, setReviews] = useState<ProductReviewsPaginatedResponse | null>(null)
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-8 rounded-lg border border-gray-200 bg-white p-8 lg:grid-cols-5">
            {/* Cột trái: Ảnh - 2 cột */}
            ...
            <div className="space-y-8 lg:col-span-3">
              ...
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2 border-r border-gray-300 pr-6">
                  ...
                  <div className="flex gap-0.5">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <svg
                        key={star}
                        className={`h-5 w-5 ${star <= product.ratingAvg ? 'text-yellow-400' : 'text-gray-300'}`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                ...
                <div className="border-r border-gray-300 pr-6">
                  <span className="font-medium text-gray-900">{product.ratingCount}</span> <span className="text-gray-600">Đánh giá</span>
                </div>
                <div>
                  <span className="font-medium text-gray-900">{product.soldQuantity}</span> <span className="text-gray-600">Đã bán</span>
                </div>
              </div>

              {/* Giá */}
              <div className="bg-gray-50 px-5 py-4 rounded-lg">
                <div className="text-5xl font-bold text-red-600">{formatPrice(currentPrice)}</div>
              </div>

              {/* Phân loại - Layout ngang */}
              <div className="flex items-start gap-8">
                <div className="w-24 shrink-0 pt-2 text-base font-medium text-gray-700">Phân loại</div>
              ...
              <div className="flex items-center gap-8">
                <div className="w-24 shrink-0 text-base font-medium text-gray-700">Số lượng</div>
                <div className="flex-1">
                  <QuantitySelector
                    quantity={quantity}
              ...
              <div className="flex gap-4 pt-2">
                <button 
                  ...
                  className="rounded-lg border border-[#004643] bg-white px-12 py-6 font-semibold text-[#004643] transition-all hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button 
                  ...
                  className="rounded-lg bg-[#004643] px-16 py-6 font-semibold text-white shadow-lg transition-all hover:bg-[#003a37] hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <span className="flex items-center justify-center gap-2">
                    <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6">
            <h2 className="mb-6 text-2xl font-bold text-gray-900">Đánh Giá Sản Phẩm</h2>
            ...
            {reviewsLoading ? (
                  ...
                  <div className="mt-6">
                    <Pagination>
                      <PaginationContent>
                        ...
                        {Array.from({ length: reviews.meta.totalPages }, (_, i) => i + 1).map((page) => (
                        ...
                        <PaginationItem>
                          <PaginationNext
                            onClick={() => setCurrentPage((prev) => Math.min(reviews.meta.totalPages, prev + 1))}
                            className={currentPage === reviews.meta.totalPages ? 'pointer-events-none opacity-50' : 'cursor-pointer'}
                          />
                        </PaginationItem>
                      </PaginationContent>
                    </Pagination>
              ...
              <div className="py-12 text-center text-gray-500">Không có đánh giá</div>
            )}
          </div>
        </div>
      </div>
